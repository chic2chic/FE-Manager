...
async function loginAndGetAuth() {
  const browser = await puppeteer.launch({
    headless: true, // CI에서는 headless true로 변경
    args: ["--no-sandbox", "--disable-dev-shm-usage"],
  });
  const page = await browser.newPage();
...
async function runLighthouseWithAuth(url) {
  // URL 파라미터 디버깅
  console.log("받은 URL 파라미터:", url);
  console.log("URL 타입:", typeof url);
  console.log("URL 구조:", JSON.stringify(url, null, 2));
  ...
  if (typeof url === "string") {
    ...
    // URL 객체인 경우 href 속성 사용
    urlString = url.href || url.url || url.toString();
  } else {
    throw new Error(`Invalid URL parameter: ${url}`);
  }

  console.log("변환된 URL 문자열:", urlString);

  const needsAuth = ["/popup-list", "/dashboard"];
  const requiresLogin = needsAuth.some(path => urlString.includes(path));

  let authData = null;

  if (requiresLogin) {
    console.log(`${urlString} - 인증 필요, 로그인 수행 중...`);
    authData = await loginAndGetAuth();
  } else {
    console.log(`${urlString} - 인증 불필요`);
  }
  ...
  try {
    ...
    const result = await lighthouse(urlString, {
      port: chrome.port,
      output: "html",
    });

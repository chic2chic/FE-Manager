...
export default function QuestionnaireForm({ number, title }: Props) {
  ...
  return (
    <div
      className="flex items-start ml-[220px] gap-[52px]"
      data-testid={`questionnaire-form-${number}`}
    >
      ...
      <div className="flex flex-col items-baseline gap-[18px] mt-[14px]">
        ...
        {answers.map((answer, idx) => (
          <QuestionnaireInput
            ...
            data-testid={`questionnaire-input-${number}-${idx}`}
        ...
        {answers.length < 5 && (
          <motion.button
            ...
            data-testid={`questionnaire-add-button-${number}`}
          >
